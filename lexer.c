...
static struct position lex_file_positionition()
{
...
struct token* token_create(struct token* _token)
{
    ...
    tmp_token.position = lex_file_positionition();
    return &tmp_token;
}
...
static struct token* token_make_string(char start_delim, char end_delim)
{
    ...
    for(; c != end_delim && c!= EOF; c = nextc())
    {
        if(c == '\\')
        {
            // We need to handle and and escape character.
            continue;
        }
        buffer_write(buffer, c);
    }
...
static struct token* token_make_identifier_or_keyword()
{
    struct buffer* buffer = buffer_create();
    char c = 0;
    LEX_GETC_IF(buffer, c, (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9') || c == '-');
...
int lex(struct lex_process* process)
{
    ...
    lex_process = process;
    process -> position.filename = process -> compiler -> cfile.abs_path;
